        ...
        Text { contents } => {
            ...
            if state.skip_leading_spaces {
                ...
                if state.inside_link > 0 {
                    // avoid nested links
                    push_remaining_text(buffer, text.trim_start())
                } else {
                    push_timestamped_text(buffer, text.trim_start())
                }
            } else {
                if state.inside_link > 0 {
                    // avoid nested links
                    ...
                    push_timestamped_text(buffer, &text)
                }
            }
        }
        _ => (),
...
fn push_timestamped_text(buffer: &mut String, text: &str) {
    let mut position = 0;
    if let Ok(re) = Regex::new(r"([0-9]+):([0-9]+)(?::([0-9]+))?") {
        for captures in re.captures_iter(text) {
            ...
            if let (Some(hours), Some(minutes), Some(seconds)) = (first, second, third) {
                ...
                push_text_with_links(buffer, &text[position..range.start]);
        ...
        push_text_with_links(buffer, &text[position..]);
    } else {
        push_text_with_links(buffer, text);
    }
}

pub fn push_text_with_links(buffer: &mut String, text: &str) {
    let mut finder = LinkFinder::new();
    finder.url_must_have_scheme(false);
    let mut position = 0;
    for link in finder.links(text) {
        push_remaining_text(buffer, &text[position..link.start()]);
        match link.kind() {
            LinkKind::Email => {
                buffer.push_str(format!("<a href=\"mailto:{}\">", link.as_str()).as_str());
            }
            LinkKind::Url => {
                if link.as_str().starts_with("http://") || link.as_str().starts_with("https://") {
                    buffer.push_str(format!("<a href=\"{}\">", link.as_str()).as_str());
                } else {
                    buffer.push_str(format!("<a href=\"https:{}\">", link.as_str()).as_str());
                }
            }
            _ => {
                buffer.push_str(format!("<a href=\"{}\">", link.as_str()).as_str());
    ...
    push_remaining_text(buffer, &text[position..]);
}
